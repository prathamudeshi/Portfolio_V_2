...
import WallpaperTitle from '@/components/3d/WallpaperTitle';

function LazyPanel({ isOpen, children }: { isOpen: boolean; children: React.ReactNode }) {
        ...
        return (
          ...
          >
            <PanelWindow id={id} title={title} icon={icon}>
              <LazyPanel isOpen={panel.isOpen}>{content}</LazyPanel>
            </PanelWindow>
          </ScenePanel>
        );
      })}
