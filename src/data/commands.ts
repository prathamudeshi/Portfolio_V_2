...
const commands: Record<string, { description: string; handler: CommandHandler }> = {
  ...
  whoami: {
    ...
    handler: () => ({
      lines: [
        ...
        line(''),
        line('  Type "education" or "awards" for more details.', '#64748b'),
      ],
    }),
  },
  ...
  education: {
    ...
    handler: () => ({
      lines: [
        line('🎓 Education:', ACCENT),
        line(''),
        ...((about as any).education || []).map((edu: any) => [
          line(`  ${edu.school}`, '#22d3ee'),
          line(`  ${edu.degree}`, '#cbd5e1'),
          line(`  ${edu.period}  ·  CGPA: ${edu.cgpa}`, '#94a3b8'),
          line(''),
        ]).flat(),
      ],
    }),
  ...
  awards: {
    ...
    handler: () => ({
      lines: [
        line('🏆 Awards & Recognition:', ACCENT),
        line(''),
        ...((about as any).awards || []).map((award: any) =>
          line(`  ★ ${award.title.padEnd(12)} ${award.description}`, '#cbd5e1')
        ),
        line(''),
      ],
    }),
